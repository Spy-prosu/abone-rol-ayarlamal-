...
const deleteCommandMessages = function (msg, client) { // //Spy Code
...
class Util {
	...
	static trimArray(arr, maxLen = 10) {
		if (arr.length > maxLen) {
			const len = arr.length - maxLen;//Spy Code
	...
	static base64(text, mode = 'encode') {
		...
		throw new TypeError(`${mode} is not a supported base64 mode.`);//Spy Code
	}
	...
	static async randomFromImgurAlbum(album) {//Spy Code
	...
	static tomorrow(timeZone) {
		const today = Util.today(timeZone);//Spy Code
		today.setDate(today.getDate() + 1);//Spy Code
		return today;
	}
	...
	static async awaitPlayers(msg, max, min, { text = 'join game', time = 30000 } = {}) {
		...
		return verify.map(message => message.author);//Spy Code
	}
