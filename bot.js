...
client.setMaxListeners(30)//Spy Code
...
setInterval(() => {
  http.get(`http://${process.env.PROJECT_DOMAIN}.glitch.me/`);//Spy Code
}, 280000);
...
fs.readdir('./komutlar/', (err, files) => {
    ...
    log(`${files.length} komut yüklenecek.`);//Spy Code
    files.forEach(f => {
...
client.load = command => {
    return new Promise((resolve, reject) => {
        try {
            let cmd = require(`./komutlar/${command}`);//Spy Code
            client.commands.set(command, cmd);
            cmd.conf.aliases.forEach(alias => {
                client.aliases.set(alias, cmd.help.name);
            });
...
client.login(process.env.TOKEN);//Spy Code
